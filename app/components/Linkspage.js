...
const Linkspage = async ({ username }) => {
  ...
  return (
    <div >
      <div className="main min-h-[100vh] h-fit max-w-[100vw] absolute inset-0 -z-10  bg-white [background:radial-gradient(125%_125%_at_50%_10%,#fff_40%,#63e_100%)] flex flex-col items-center pb-5">
        ...
        <div className="icons flex gap-5 my-5 relative">
          <Link className='flex justify-center items-center border-2 border-[#8f8f8f] rounded-md absolute -left-[13vw] text-[#8f8f8f] py-1 px-4' href={"/"}>Back</Link>
        ...
        {newarr ? newarr.map((obj) => {
          ...
          return <div key={obj.link} className="shadow border-2 border-black h-12 w-[45vw] mx-auto rounded-lg mt-5 bg-black relative">
          <Link  href={obj.link} target='_blank' >
            <div className="link bg-white border-2 border-black h-12 w-[45vw] mx-auto rounded-lg mt-5 flex justify-center items-center text-xl text-purple-700 absolute bottom-2 right-2">
              {obj.linktext}
            </div>
          </Link>
          </div>
